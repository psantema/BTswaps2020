...
Sys.setenv(TZ='Germany/Berlin')
con= dbcon(user = 'psantema', pwd = 'psantema2405', host = 'scidb.mpio.orn.mpg.de')


fledge = dbq(con, enhance=FALSE, glue('SELECT site fledge_box, max(datetime_) fledge, date(max(datetime_)) fledge_date, time(max(datetime_)) fledge_time, transponder transp FROM
                             BTatWESTERHOLZ.transponders WHERE date(datetime_)<"2020-07-01" AND
                             transponder in ({paste(shQuote(chicks$transp), collapse= ",")})
                             GROUP BY transponder
                             ORDER BY site, max(datetime_)') )
write.csv(fledge, file='fledge.csv')
...
#Get delta fledging time
  ...
  d[, alone := sign(ifelse(is.na(days_to_prev),days_to_next,
               ...
               ifelse(days_to_prev<days_to_next, days_to_prev, days_to_next))))]
...
##Get data overview
  ...
  wilcox.test(e$fledge_age[e$group=='older' & e$treat=='treatment'],
              e$fledge_age[e$group=='older' & e$treat=='control'],
              paired=TRUE)
  wilcox.test(e$fledge_age[e$group=='younger' & e$treat=='treatment'],
              e$fledge_age[e$group=='younger' & e$treat=='control'],
              paired=TRUE)
